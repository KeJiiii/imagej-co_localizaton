...
for (times = 0; times < 4 ; times++) {
		...
		for (l = 0; l < son_file_list.length; l++){
			if (startsWith(son_file_list[l],"rem") && endsWith(son_file_list[l],"tif")){
				...
				for (i = 1; i < 3; i++){
					...
					run("Subtract Background...", "rolling=200 stack");
					run("Gaussian Blur...", "sigma=0.60 stack");
					run("8-bit");
					if (i==1) run("Auto Local Threshold", "method=Phansalkar radius=10 parameter_1=0 parameter_2=0 white stack");
					if (i==2) run("Auto Local Threshold", "method=Phansalkar radius=10 parameter_1=0 parameter_2=0 white stack");
					
					//run("Convert to Mask", "method=IsoData background=Dark black");  //这里 313不能有black 但是笔记本可以
					 
					run("Watershed", "stack");
					rename(i);
				}
				imageCalculator("OR create stack", "1","2");
				rename(3);
				
				selectWindow(3);
				roiManager("reset");
				//run("Analyze Particles...", "size=15-Infinity circularity=0.30-1.00 display clear include add stack");
				run("Analyze Particles...", "size="+scale+"-200 circularity="+circle1+"-"+circle2+" display clear include add in_situ stack");
				...
				roiManager("Show All");
				roiManager("Save", select_path+"ROI_"+n_1+"_"+filename+".zip");
				run("Clear Results");
	...
	function remove_zskin(fix,select_channel,move,threshold,Fill,Erode) { 
		mergestring="";
		open(fix); 
		reference=getTitle();
		...
		open(move);
		raw= getTitle();
		if (select_channel=="Ch1") run("Duplicate...", "duplicate channels=1");
		if (select_channel=="Ch2") run("Duplicate...", "duplicate channels=2");
		...
		saveAs("tiff", select_path+"Auto_"+reference1);
		close("*");
	}
	...
	function user_input(){
		...
		if (user_choice==4){
			...
			Dialog.create("Remove skin parameters");
				...
				Dialog.addNumber("Fill times", 40);
					...
					if(rem=="no"){
						for (l = 0; l < son_file_list.length; l++){
							if (startsWith(son_file_list[l],"stitch") && endsWith(son_file_list[l],".tif")){
								...
								remove_zskin(fix,select_channel,move,threshold,Fill,Erode);
							}
							close("*");
						}
					}
		...
		if (user_choice==5){
			...
			Dialog.create("Analyze Particle parameter");
				...
				Dialog.addNumber("particle size", 15);
